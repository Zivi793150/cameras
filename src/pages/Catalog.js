...
const Catalog = () => {
  ...
  const getCardLink = (product) => {
    // Для мастер-товаров и productGroup всегда ведём на /catalog или /catalog/категория
    if (product && (product.isMaster || product.productGroup)) {
      if (product.categorySlug) return `/catalog/${product.categorySlug}`;
      return `/catalog`;
    }
    // Обычные товары
  ...
  return (
    <div className="catalog">
      <Seo
        title="Каталог товаров — Eltok.kz"
        description="Каталог электроинструментов и оборудования ТАНКЕР: болгарки, шуруповёрты, перфораторы, дрели, лобзики и другое. Доставка по Казахстану."
        canonical="https://eltok.kz/catalog"
        jsonLd={{
      ...
      <main className="catalog-main">
        <div className="container catalog-layout">
          <div className="catalog-content" style={{maxWidth: '1100px', margin: '0 auto', width: '100%'}}>
            {/* Хлебные крошки как на странице продукта */}
            <nav className="breadcrumbs" style={{paddingBottom: '12px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '8px', justifyContent: 'center'}}>
              <a href="/">Главная</a>
              <span style={{margin: '0 8px', color: '#bdbdbd', fontSize: '18px'}}>&rarr;</span>
              <a href="/catalog">Каталог</a>
            </nav>
            <h1 className="catalog-title" style={{textAlign: 'center', marginLeft: 0, marginTop: 0}}>
              Каталог товаров
            </h1>
            {loading ? (
                  ...
                  >
                    ...
                    <div className="product-info" style={{padding: '0 8px 6px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0, minHeight: '20px'}}>
                          <span style={{fontSize: '1rem', fontWeight: 700, color: '#1a2236', margin: '0', lineHeight: 1.2, textDecoration:'none',cursor:'pointer',display:'block', textAlign:'center', width:'100%', marginTop: '2px'}}>
                            {product.productGroup ? (product.productGroup.catalogName || product.productGroup.name) : (product.category ? product.category : product.name)}
                          </span>
                    </div>
                  </div>
                </Link>
